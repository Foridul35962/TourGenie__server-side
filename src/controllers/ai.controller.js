...
export const searchField = AsyncHandler(async (req, res) => {
    ...
    const model = genAI.getGenerativeModel({
        ...
        systemInstruction: `
            ...
            - location:
            - If prompt includes origin and destination like "Dhaka to Cox's Bazar", capture the route as a single string: "Dhaka to Cox's Bazar".
            - If only one place is mentioned like "Cox's Bazar", capture that place as location.
            - Keep casing and punctuation reasonably close to user text (normalize minor spacing only).
            - budgetType:
            - Map synonyms:
                - cheap / low / low budget / budget / economy / chip => "cheap"
...
const makeCacheKey = ({ location, budget, members, days, prompt }) => {
    const base = [
        normalize(location),
        normalize(budget),
        members,
...
export const createPlan = AsyncHandler(async (req, res) => {
    const { location, budget, members, days, prompt } = req.body;
    if (
        location == null ||
        budget == null ||
        members == null ||
    ...
    //redis check
    const cacheKey = makeCacheKey({ location, budget, members, days, prompt });
    ...
    const model = genAI.getGenerativeModel({
        ...
        systemInstruction: `
            ...
            - Costs must be realistic for Bangladesh context unless location implies otherwise.
    ...
    try {
        ...
        return res
            .status(200)
            .json(
                new ApiResponse(200, reply, 'trip plan successfully')
            )
    } catch (error) {
        throw new ApiErrors(500, error.message)
    }
